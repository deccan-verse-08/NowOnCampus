...
# Cron security key (used by event reminder job endpoint)
...
4. Set `CRON_SECRET` in Vercel Environment Variables so scheduled reminder calls are authenticated.
5. Keep the included `vercel.json` cron config enabled (`0 * * * *`) to trigger `/api/cron/event-reminders` hourly.
6. Set the Custom Install Command to `npm install` and the Build Command to `npm run build`.
7. Deploy!
